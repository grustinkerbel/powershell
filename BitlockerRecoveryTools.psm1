    ...
    if (-not $IsEscrowed) {
        ...
        try {
            if (-not $IsWhatIf) {
                ...
                Write-Log "WhatIf: Would back up BitLocker key $RecoveryKeyID" -ComputerName $Status.Computer
            }
        }
        catch {
        ...
        Write-Log "Recovery key already escrowed — no backup needed" -ComputerName $Status.Computer
        $ADVerified = $true
    }
